...
--<<<CLAUSTROPHOBIA>>>--
function Exodus:claustrophobiaUpdate()
    local player = Isaac.GetPlayer(0)
...
Exodus:AddCallback(ModCallbacks.MC_POST_UPDATE, Exodus.claustrophobiaUpdate)

function Exodus:claustrophobiaNewRoom()
    local player = Isaac.GetPlayer(0)
    local room = game:GetRoom()
...
Exodus:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, Exodus.claustrophobiaNewRoom)

function Exodus:claustrophobiaCache(player, flag)
    if player:HasCollectible(ItemId.CLAUSTROPHOBIA) then
        if flag == CacheFlag.CACHE_SPEED then
...
Exodus:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, Exodus.claustrophobiaCache)
